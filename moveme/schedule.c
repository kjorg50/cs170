...
int do_noquantum(message *m_ptr)
{
	...
	// If there was a problem running the lottery, return the error
...
static int run_lottery(){
	...
	for (proc_nr=0, rmp=schedproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
		if(rmp->priority == USER_Q && (rmp->flags & IN_USE) ){
			...
			//[debug]
			printf("found user procnum: %d , in lotto sum: %d\n",proc_nr, ticket_totals_sum);
		}
	}
	proc_nr = -1;

	srandom((unsigned) time(NULL));
	win_val = random() % ((int)ticket_totals_sum);
	for (proc_nr=0, rmp=schedproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
		if((rmp->flags & IN_USE) && rmp->priority==USER_Q){
			...
			if(win_val >= 0){
				...
				if(win_val < 0){
					...
					//[debug]
					printf("Winning lottery value: %d\n",win_val);
			...
			if(old_priority != rmp->priority){
				schedule_process(rmp,error_flag);
			}
		}
	}
